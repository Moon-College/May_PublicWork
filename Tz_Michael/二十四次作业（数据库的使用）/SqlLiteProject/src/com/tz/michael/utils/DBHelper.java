...
public class DBHelper {
	 ...
	 * @param id
	 */
	public static void updateData(SQLiteDatabase db,String table,Object object,int id){
		ContentValues values=new ContentValues();
		//拿到磨具类
		Class clazz=object.getClass();
		Field[] fields = clazz.getDeclaredFields();
	 ...
	 * @return
	 */
	public static Object queryData(SQLiteDatabase db,String table,Class clazz,int id){
		...
		try {
			...
			for(Field field:fields){
				...
				cursor.moveToFirst();//这里因为是根据id来差，id唯一，所以结果只有一行数据
					String v_Str=cursor.getString(cursor.getColumnIndex(fieldName));
	 ...
	 * @return
	 */
	public static List<Object> queryDataByName(SQLiteDatabase db,String table,Class clazz,String name){
	...
	public static void queryDataTest(Object object){
		...
		for(Field field:fields){
			try {
				field.setAccessible(true);
				Log.i("field--", field.getName()+field.get(object)+"---"+field.getType().getName());
			} catch (IllegalArgumentException e) {
				e.printStackTrace();
			} catch (IllegalAccessException e) {
				e.printStackTrace();
