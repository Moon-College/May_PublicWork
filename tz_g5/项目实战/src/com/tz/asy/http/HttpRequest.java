...
//		List<NameValuePair> parameters = new ArrayList<NameValuePair>();
    ...
    private AysResponse doQuestByGet(String url, HttpParams params, boolean isCookie, HttpCallback callback) {
        AysResponse response = new AysResponse();
        ...
        if (params != null) {
            ...
            for (Field field : declaredFields) {
                ...
                try {
                    Object value = field.get(params);
                    if (value != null) {
                        urlWithParams.append(name);
                        urlWithParams.append("=");
                        urlWithParams.append(value);
                        urlWithParams.append("&");
                    }
                } catch (IllegalArgumentException e) {
                    // TODO Auto-generated catch block
            ...
            urlWithParams.deleteCharAt(url.length() - 1);
        }
        HttpGet get = new HttpGet(urlWithParams.toString());
        if (isCookie) {
            //经常用来登陆验证
            String value = (String) MyDataUtils.getData(context, MyConstants.COOKIE, MyConstants.SETCOOKIE, String.class);
            get.addHeader(MyConstants.SETCOOKIE, value);//把cookie带到服务端
        }
        setTimeOutException();
        HttpResponse resp = null;
        try {
...
//	}
    ...
    public void doQuestByGetMethod(String url, HttpParams params,
        ...
        task.execute(url, params, isCookie, "GET", callback);
    }
