...
public class MainActivity extends Activity implements OnItemClickListener {
	...
	private void initData(String path) {
		...
		if (!file.exists()) {
			...
			backFile.setFileBmp(BitmapFactory.decodeResource(getResources(), R.drawable.dirs));
			data.add(backFile);
			...
			for (File f : files) {
				...
				if (f.isDirectory()) {
					//是目录
					sdFile.setFileBmp(BitmapFactory.decodeResource(getResources(), R.drawable.dirs));
					sdFile.setDirectory(true);
				} else if(f.getName().toLowerCase().endsWith(".png") || f.getName().toLowerCase().endsWith(".jpg")) {
					//因为图片需要压缩，因此先设为null
					sdFile.setFileBmp(null);
					sdFile.setDirectory(false);
				} else {
					//是文件
					sdFile.setDirectory(false);
					sdFile.setFileBmp(BitmapFactory.decodeResource(getResources(), R.drawable.file));
				}
				data.add(sdFile);
